...
const getDebts = async currentUserData => {
	try {
		const items = await dbFunctions.find(debtsModel, {
			search: { isCompleted: false },
			sort: { date: -1 },
			populate: populatePerson
		})

		if (items?.length === 0) return []

		const currentUser = await dbFunctions.findOne(usersModel, currentUserData?._id?.toString(), {
			populate: populatePerson
		})
...
exports.getDashboard = async (req, res) => {
	try {
		...
		if (isNaN(currentMonth) || !currentYear) res.status(400).json({ status: 'error', message: 'Missing params' })

		const search = currentMonthSearch(currentMonth, currentYear)
		const [
			{
				category: categoryData = { ...emptyDataForCharts },
				subCategory: subCategoryData = { ...emptyDataForCharts }
			},
			...
			getCurrentMonthTransactions(currentMonth, currentYear, {
				replaceFields: true,
				showAll: false,
				sort: { amount: -1 }
			})
...
exports.convertCurrency = async (req, res) => {
	try {
		const errors = []
		if (!req?.body.fromType) errors.push('fromType')
		if (!req?.body.toType) errors.push('toType')
		if (!req?.body.sourceAmount) errors.push('sourceAmount')
		if (!req?.body.finalAmount) errors.push('finalAmount')
		...
		const subCategories = await dbFunctions.find(subCategoriesModel, {
			search: { name: getIlikeSearch('Conversión de Monedas') }
		})
		...
		if (responses?.[0]?.status === 'error' || responses?.[1]?.status === 'error')
			return res
				.status(500)
				.json({ status: 'error', message: responses?.[0]?.message ?? responses?.[1]?.message })
...
const defaultTransactionValueExists = (categories, subCategories, dtv, item) => {
	...
	const subCategoryUid = subCategories
		.find(s => s.name === item.subCategory && s.category?.toString() === categoryUid)
		?._id?.toString()
...
const checkDefaultTransactionValuesExists = async (categories, subCategories) => {
	...
	if (missingDTV?.length > 0) await dbFunctions.insertMany(dtvModel, missingDTV)
}
...
const checkMissingItemExists = async (model, dataList, jsonValues = [], modelName, checkField = 'name') => {
	try {
		...
		if (missingItems?.length > 0)
			if (modelName === 'users') {
				const roles = await dbFunctions.find(rolesModel, {
					search: { $or: missingItems.map(i => ({ name: i.role })) }
				})
				if (roles.length > 0)
...
const checkSubCategoriesExists = async (categories, subCategories) => {
	if (categories && subCategories) {
		...
		if (missingSubCategories?.length > 0) await dbFunctions.insertMany(subCategoriesModel, missingSubCategories)

		const modifiedSubcategories =
			missingSubCategories?.length > 0 ? await dbFunctions.find(subCategoriesModel) : [...subCategories]

		await checkDefaultTransactionValuesExists(categories, modifiedSubcategories)
	}
}
	...
	try {
		if (req?.body?.model) {
			...
			for (let index = 0; index < items.length; index++) {
				...
				const created_at = moment(temp?.date)
					.set({ milliseconds: index + 1 })
					.valueOf()
				const updateResponse = await dbFunctions.updateOne(model, id, { created_at })
				if (updateResponse) response.push(updateResponse)
			}
