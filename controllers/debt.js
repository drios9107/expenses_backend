...
const {
	...
	populatePerson,
	getIlikeSearch,
	getProperDebtCategoryOrSubCategory,
...
exports.create = async (req, res) => {
	...
	try {
		...
		const [categories, subCategories] = await Promise.all([
			dbFunctions.find(categoryModel, {
				session,
				search: { $or: [{ name: getIlikeSearch('Ingresos') }, { name: getIlikeSearch('Otros') }] }
			}),
			dbFunctions.find(subCategoryModel, {
				session,
				search: {
					$or: [{ name: getIlikeSearch('Préstamos') }, { name: getIlikeSearch('Devolución de Préstamos') }]
				}
			})
		])
		...
		if (transactionResponse?.status === 'error') return res.status(500).json(transactionResponse)
...
exports.delete = async (req, res) => {
	...
	try {
		...
		if (transactionsResponse?.status === 'error') return res.status(500).json(transactionsResponse)
		...
		if (req?.body?.isCompleted) {
			const [categories, subCategories] = await Promise.all([
				...
				dbFunctions.find(subCategoryModel, {
					...
					search: {
						$or: [
							{ name: getIlikeSearch('Préstamos') },
							{ name: getIlikeSearch('Devolución de Préstamos') }
						]
					}
				})
			])
			...
			const data = [
				generateDebtCompletedTransaction(categories, req?.body?.isMyDebt, subCategories, commonPayload)
			]
			const transactionResponse = await dbFunctions.insertMany(transactionModel, data, { session })
